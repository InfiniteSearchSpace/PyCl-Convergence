__kernel void RunAutomata(const int ySize, __global int* a, __global int* c)
{
	...
	int c_out = a[my_id];
	if(count >= 180) {c_out = 0;}
	//if(count >= 270 && count <= 290) {c_out = a[my_id]+1;}
	if(count >= 185 && count <= 188) {c_out = a[my_id]+1;}
	if(count >= 120 && count <= 128) {c_out = a[my_id]+1;}
	if(count <= 120) {c_out = 0;}
	//if(count >= 90 && count <= 93) {c_out = a[my_id]+1;}
	if(count >= 68 && count <= 80) {c_out = a[my_id]+1;}
