...
//:0,999,0,:77,133,1,:39,47,1,:11,11,1,
	...
	int c_out = a[my_id];
    if(count <= 32) {c_out = 0;}
	if(count >= 30 && count <= 35) {c_out = a[my_id] + 1;}
	if(count >= 42 && count <= 60) {c_out = 0;}
	if(count >= 70 && count <= 80) {c_out = a[my_id] + 1;}
	if(count >= 89 && count <= 110) {c_out = 0;}


	if(c_out >= 768) {c_out = 768;}
	c[my_id] = c_out;
