...
//Get gliders to be birthed from wave's crystal form
	...
	if(count >= 100) {c_out = 0;}
	if(count >= 80 && count <= 99) {c_out = a[my_id]+1;}
	if(count >= 65 && count <= 79) {c_out = 0;}
	if(count >= 30 && count <= 55) {c_out = a[my_id]+1;}
	...
	if(count >= 0 && count <= 6) {c_out = 0;}
	if(count >= 16 && count <= 20) {c_out = a[my_id]+1;}	
