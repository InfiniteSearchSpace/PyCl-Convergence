...
//Get gliders to be birthed from wave's crystal form
	...
	if(count >= 117 && count <= 190) {c_out = a[my_id]+1;}
	if(count >= 68 && count <= 78) {c_out = a[my_id]+1;} 
	if(count >= 70 && count <= 73) {c_out = 0;} 
	if(count >= 40 && count <= 42) {c_out = a[my_id]+1;}
	//if(count >= 31 && count <= 39) {c_out = 0;}
	...
	if(count >= 14 && count <= 18) {c_out = a[my_id]+1;}	
