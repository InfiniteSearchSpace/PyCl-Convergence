...
//Get gliders to be birthed from wave's crystal form
	...
	if(count >= 22 && count <= 75) {c_out = 0;}
	if(count >= 68 && count <= 71) {c_out = a[my_id]+1;}
	if(count >= 95 && count <= 115) {c_out = 0;}
	if(count >= 137 && count <= 149) {c_out = a[my_id]+1;}
