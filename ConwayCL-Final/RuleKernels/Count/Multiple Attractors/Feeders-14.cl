__kernel void RunAutomata(const int ySize, __global int* a, __global int* c)
{
	...
	if(count >= 85 && count <= 145) {c_out = a[my_id]+1;}

	if(count >= 40 && count <= 49) {c_out = 0;}
	if(count >= 37 && count <= 37) {c_out = a[my_id]+1;}
	if(count >= 175) {c_out = 0;}
	if(count >= 121 && count <= 121) {c_out = 0;}
	if(count >= 210 && count <= 295) {c_out = a[my_id]+1;}
