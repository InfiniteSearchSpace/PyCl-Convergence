__kernel void RunAutomata(const int ySize, __global int* a, __global int* c)
{
	...
	int c_out = a[my_id];
	if(count >= 158) {c_out = 0;}
	if(count >= 114 && count <= 119) {c_out = a[my_id]+1;}
	if(count == 77 || count == 83) {c_out = a[my_id]+1;}
	if(count <= 12 || count >= 50 && count <= 80) {c_out = 0;}
	if(count == 108) {c_out = 0;}
	if(count == 47 || count == 1) {c_out = a[my_id]+1;}
