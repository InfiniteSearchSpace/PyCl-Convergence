__kernel void RunAutomata(const int ySize, __global int* a, __global int* c)
{
	...
	int c_out = a[my_id];
	if(count >= 30 && count <= 50) {c_out = 0;}
	//if(count >= 215 && count <= 230) {c_out = a[my_id]+1;}
	if(count >= 30) {c_out = 0;}
	//if(count >= 70 && count <= 73) {c_out = a[my_id]+1;}
	if(count >= 65 && count <= 66) {c_out = a[my_id]+1;}
	//if(count >= 125 && count <= 125) {c_out = a[my_id]+1;}
