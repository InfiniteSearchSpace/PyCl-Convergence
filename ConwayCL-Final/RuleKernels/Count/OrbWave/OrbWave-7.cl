__kernel void RunAutomata(const int ySize, __global int* a, __global int* c)
{
	...
	int c_out = a[my_id];
	if(count >= 130 && count <= 180) {c_out = a[my_id]+1;}
	if(count >= 190) {c_out = 0;}
	if(count >= 115 && count <= 117) {c_out = a[my_id]+1;}
	if(count >= 78 && count <= 83) {c_out = a[my_id]+1;}
	if(count <= 12 || count >= 50 && count <= 76) {c_out = 0;}
	if(count >= 108 && count <= 135) {c_out = 0;}
	if(count == 47 || count == 23) {c_out = a[my_id]+1;}
